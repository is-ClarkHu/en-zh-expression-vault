...
- SYMBOLS & PUNCTUATION & EMOJI: if the term is a symbol, punctuation mark, or emoji the user wants to know how to SAY OUT LOUD (e.g. "/", "\\", "#", "&", "@", "%", "~", "^", "*", "—", "…", "•", "©", "🔥"), this is the ONE allowed exception to "never substitute": return an ordinary card (kind "word", pos "noun") whose surface is the most common spoken English NAME of the symbol — "/" → "slash", "\\" → "backslash", "#" → "hash", "&" → "ampersand", "@" → "at sign", "*" → "asterisk", "~" → "tilde", "^" → "caret". Set gloss_cn to the Chinese name with the symbol in parentheses (e.g. "斜杠（/）"), put the literal symbol in intent_cn context (e.g. "符号 / 的英文读法"), and list common alternate names as synonym relations (e.g. "/" → relations: [{ "type":"synonym", "surface":"forward slash", "register":"neutral", "common":false }]). Never return an empty result for a symbol the user typed.
